...
- Message는 Task를 직접 수행하지 않고, Task를 담아 전달하는 구조화된 운반 객체다.
- 즉, `message.task`는 Task의 실제 데이터 페이로드이며, `message.message_type`은 전달 메시지의 종류다.
- Message는 통신 계층, Task는 작업 계층이다.
- 일반적으로 Task가 Missing되면 `task_id`, `title`, `objective`, `assigned_agent`만 들어 있어도 된다.

#### message → task 관계 규칙
- Task는 반드시 Message 안에 `task` 필드로 실려야 한다.
- Message는 `TASK`, `REQUEST`, `RESULT`, `REVIEW_REQUEST`, `ERROR`, `APPROVAL_REQUEST` 등으로 메시지 유형을 바꾸고, 그 안에서 Task 정보를 참조한다.
- 구체적으로는 다음 흐름을 따른다.

```text
Secretary/Orchestrator -> Message(TASK) -> Task(payload inside task field)
Task result -> Message(RESULT or REVIEW_RESULT) -> report back
```
...
- 이 필드는 `allowed_resources`와 달리 "읽을 수 있는 파일"이 아니라 "반드시 이해해야 하는 운영 규범 문서"로 취급한다.
- Agent는 `required_protocol_knowledge`에 포함된 문서 경로를 참조할 때, 수동으로 문서의 제목만 읽는 것이 아니라 내용 전반을 파악하고, 해당 규칙이 Task에 반영되는지 확인해야 한다.
...
- 프로젝트 내부의 workflow 경로는 `workflow/projects/<project_id>/` 형태로 통일한다.
...
status: active
workflow:
  root: workflow/projects/PROJECT-20260913-0001
  planning: true
  execution: true
  review: true
  reporting: true
...
### 3.5 workflow 접근 권한 규칙

Agent는 workflow 경로에 접근할 때 다음 원칙을 따른다.

1. Agent는 직접 전체 프로젝트 workflow에 접근하지 않는다.
2. Task의 `allowed_resources`에 포함된 경로만 읽기/쓰기 범위로 허용한다.
3. workflow의 `planning`, `execution`, `review`, `reporting` 중 필요한 단계만 접근한다.
4. 파일 생성, 수정, 삭제, 코드 변경, 배포, 외부 API 호출은 별도 사용자 승인과 검증을 요구한다.
5. 작업 수행 완료 후 결과는 Message 또는 Report로 작성하고, 필요한 경우 `workflow/projects/<project_id>/reporting/reports/` 또는 `user_reports/`에 남긴다.

### 3.6 workflow 저장 구조와 Agent의 셋업 의무

workflow는 여러 프로젝트를 저장할 수 있는 디렉터리 구조다. 이 구조에서 Agent는 다음 단계를 기반으로 작업을 수행한다.
...
- `planning`: 요구사항, 설계, Task 분해.
- `execution`: 구현, 테스트, 산출물.
- `review`: 검토 요청, 검토 결과, 보안/품질 체크.
- `reporting`: 실행 결과/요약/최종보고.

Agent는 `workflow/projects/<project_id>/...` 경로에 따라 작업 결과를 저장하고, 별도의 Task 결과는 Message의 `RESULT` 타입으로 반환한다.
